...
/**
 * Unit tests for {@link Report000v2Service}.
 */
@ExtendWith(MockitoExtension.class)
class Report000v2ServiceTest {

  private Report000v2Service service;
  ...
  void setUp() {
    ...
    service = new Report000v2Service(jdbcTemplate, s3ClientWrapper, creationService, metricsHandler, fixedClock, appConfig);
  }
  ...
  void generateReport_shouldCallTheRightServicesWithTheRightValues() {
    when(appConfig.isForceRunReport000v2()).thenReturn(false);
    ...
    verify(s3ClientWrapper).uploadFile(any(File.class), eq("reports/monthly/report_000_v2_2025-12-21.csv"));
  }
  ...
  void returnsFalseOnThe22ndOfMonth() {
    ...
    fixedClock = Clock.fixed(fixedNow, ZoneOffset.UTC);
    Report000v2Service service = new Report000v2Service(jdbcTemplate, s3ClientWrapper, creationService, metricsHandler, fixedClock, appConfig);
  ...
  void returnsTrueOnThe22ndOfMonthIfOverrideSet() {
    ...
    when(appConfig.isForceRunReport000v2()).thenReturn(true);
