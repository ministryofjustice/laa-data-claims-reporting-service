...
public class CsvCreationServiceTest {
  ...
  @InjectMocks private CsvCreationService csvCreationService;
  ...
  void shouldThrowIllegalArgumentExceptionWhenSqlIsNull() {
    assertThrows(
        ...
        () -> csvCreationService.buildCsvFromData(null, bufferedWriter, "test_report"));
  }
  ...
  void shouldThrowIllegalArgumentExceptionWhenSqlIsEmpty() {
    assertThrows(
        CsvCreationException.class,
        () -> csvCreationService.buildCsvFromData("", bufferedWriter, "test_report"));
  }
  ...
  void shouldThrowIllegalArgumentExceptionWhenBufferedWriterIsNull() {
    assertThrows(
        CsvCreationException.class,
        () ->
            csvCreationService.buildCsvFromData(
                "SELECT * FROM ANY_REPORT.DATA", null, "test_report"));
  }

  @Test
  void shouldExecuteQueryWhenValidParametersProvided() {
    csvCreationService.buildCsvFromData(
        "SELECT * FROM ANY_REPORT.DATA", bufferedWriter, "test_report");
    verify(jdbcTemplate)
        .query(any(PreparedStatementCreator.class), any(CsvRowCallbackHandler.class));
  }
