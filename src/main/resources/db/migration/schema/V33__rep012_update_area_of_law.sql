CREATE OR REPLACE FUNCTION claims.refresh_report012()
    RETURNS void
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
    ...
    EXECUTE 'DROP TABLE IF EXISTS claims.report_012';
    ...
    IF NOT report_data_exists THEN
            ...
            ----------------------------------------------------------------
            EXECUTE '
                CREATE TABLE claims.report_012 (
    ...
    EXECUTE 'CREATE TABLE IF NOT EXISTS claims.report_012 (
                "Provider Office Account Number" TEXT,
                "Area of Law" TEXT,
                ' || column_definitions || '
            )';
    ...
    insert_sql := 'INSERT INTO claims.report_012
                SELECT
