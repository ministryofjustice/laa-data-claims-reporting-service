    ...
    data_query := $dq$
            ...
            claim_totals AS (
                SELECT c.submission_id,
                       c.id AS claim_id,
                       CASE
                           WHEN a_latest.claim_id IS NULL THEN COALESCE(cf_latest.total_amount, 0)
                ...
                LEFT JOIN LATERAL (
                    SELECT cf.total_amount
                    FROM claims.calculated_fee_detail cf
                    WHERE cf.claim_id = c.id
                    ORDER BY COALESCE(cf.updated_on, cf.created_on) DESC NULLS LAST, cf.id DESC
                    LIMIT 1
                ) cf_latest ON TRUE
                GROUP BY c.submission_id, c.id, a_latest.claim_id, cf_latest.total_amount
            ),
