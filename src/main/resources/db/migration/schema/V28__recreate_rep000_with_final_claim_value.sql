...
SELECT
    ...
    COALESCE(c.is_amended::text, '')                                        AS "Amended Flag",
    COALESCE(c.has_assessment::text, '')                                    AS "Assessed Flag",
    ...
    COALESCE(a.id::text, '')                                                AS "Claim Assessment ID",
    COALESCE(a.assessment_outcome::text, '')                                AS "Assessment Outcome",
    ...
    COALESCE(a.disbursement_vat_amount::text, '')                           AS "Disbursement VAT Amount",
    ...
    COALESCE(a.is_vat_applicable::text, '')                                 AS "Is VAT Applicable",
    ...
    COALESCE(a.allowed_total_incl_vat::text,
         COALESCE(
                 CASE
                     WHEN calc.total_amount::text ~ '^[\s+-]?\d+(\.\d+)?$' THEN ROUND(calc.total_amount, 2)::text
                     ELSE NULL
                     END, '')
    ) AS "Final Claim Value"
