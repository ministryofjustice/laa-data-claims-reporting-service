...
FROM submission_periods AS sp
    ...
    -- LATERAL join for latest claim summary fee (prevents duplicates after amendments)
    LEFT JOIN LATERAL (
    SELECT *
    FROM claims.claim_summary_fee csf_inner
    WHERE csf_inner.claim_id = c.id
    ORDER BY COALESCE(csf_inner.updated_on, csf_inner.created_on) DESC NULLS LAST, csf_inner.id DESC
    LIMIT 1
    ) csf ON TRUE
