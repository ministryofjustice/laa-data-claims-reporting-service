...
SELECT
    ...
    COALESCE(TO_CHAR(al.assessment_created_on, 'DD/MM/YYYY'), '') 	                            AS "Amendment Date",
    COALESCE(TO_CHAR(al.assessment_created_on, 'HH24:MI:SS'), '') 	                            AS "Amendment Time",
    ...
    -- can remove default when assessments have been backfilled with types and reasons.
    claims.convert_string_to_title_case(COALESCE(al.assessment_type, 'ESCAPE_CASE_ASSESSMENT'))	AS "Assessment Type",
    COALESCE(al.assessment_reason, 'Escape Fee Case Assessment')						        AS "Assessment Reason",
