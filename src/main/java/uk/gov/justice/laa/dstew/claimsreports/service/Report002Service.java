...
public class Report002Service extends AbstractReportService {
  ...
  protected boolean runToday() {
    if (appConfig.isForceRunReport002()) {
      ...
      log.info("Force run for Report002 is disabled. Running report only if today is the {}th of the month.", MONTHLY_REPORT_DATE);
    }
    return appConfig.isForceRunReport002() || LocalDate.now(clock).getDayOfMonth() == MONTHLY_REPORT_DATE;
  }
}
