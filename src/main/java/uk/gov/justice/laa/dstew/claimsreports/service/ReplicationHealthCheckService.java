...
/**
 * The {@code ReplicationHealthCheckService} class provides functionality to evaluate the health of
 * replication processes within a database environment. This service checks multiple parameters and
 * reports on the system's replication status, including checks for missing tables, write-ahead log
 * (WAL) progression discrepancies, and differences in expected vs actual data counts.
 *
 * <p>The service is responsible for: - Fetching relevant database state (e.g., published tables,
 * replication summaries). - Running detailed health checks for replication issues. - Generating a
 * comprehensive report capturing the replication's health status and failures, if any.
 */
@Service
...
public class ReplicationHealthCheckService {
  ...
  /**
   * Checks the replication health for a specific date, typically the previous day. This method
   * evaluates various metrics and conditions such as missing tables, write-ahead log (WAL)
   * progress, and data counts to determine the overall health of the system's replication
   * processes.
   *
   * <p>@return A {@code ReplicationHealthReport} object containing the health status, any detected
   * issues, and a summary of the replication's health for the checked date.
   */
  public ReplicationHealthReport checkReplicationHealth() {
    ...
    if (publicationTables == null || publicationTables.isEmpty()) {
      ...
      report.addFailure(
          "publication", "No tables found for publication claims_reporting_service_pub");
    } else {
      report.setTableSummaryOk(true); // OK so far, i.e. at least publication tables exist.
    }

    Map<String, ReplicationSummary> summaries =
        metadataRepository.getReplicationSummaries(summaryDate);

    if (summaries == null || summaries.isEmpty()) {
      report.setTableSummaryOk(false);
      report.addFailure("replication_summary", "No replication summary found for " + summaryDate);
    }
  ...
  private void checkWalProgress(ReplicationHealthReport report) {
    ...
    if (wal == null || wal.latestEndLsn() == null) {
      ...
      report.addFailure(
          REPLICATION,
          "No WAL progress information available, replication is failing. Please check RDS logs for more details.");
    } else {
      ...
      if (lastApplied == null) {
        ...
        report.addFailure(
            REPLICATION,
            String.format("Replication apply has not progressed for %d minutes", lagMinutes));
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Table names are validated as SQL identifiers before count queries are assembled.")
  private void checkCounts(
    ...
    for (ReplicationSummary summary : summaries.values()) {
      ...
      String updatedSql =
          String.format("SELECT count(*) FROM %s WHERE updated_on BETWEEN ? AND ?", tableName);
      ...
      if (!Objects.equals(actualRecordCount, summary.recordCount())
        ...
        report.addFailure(
            summary.tableName(),
            String.format(
                "Count mismatch — expected (%d/%d), actual (%d/%d)",
                summary.recordCount(),
