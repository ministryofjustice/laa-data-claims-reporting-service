...
public class DatabaseStatisticService {
  ...
  public static final String QUERY_ACTIVE_CONNECTIONS =
      "SELECT count(*) FROM pg_stat_activity WHERE state = 'active'";
  public static final String QUERY_IDLE_CONNECTIONS =
  ...
  public void setDatabaseMetrics() {
    ...
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_TOTAL, totalConnections != null ? totalConnections : 0);

    var activeConnections = jdbcTemplate.queryForObject(QUERY_ACTIVE_CONNECTIONS, Double.class);
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_ACTIVE, activeConnections != null ? activeConnections : 0);

    var idleConnections = jdbcTemplate.queryForObject(QUERY_IDLE_CONNECTIONS, Double.class);
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_IDLE, idleConnections != null ? idleConnections : 0);

    var maxConnections = jdbcTemplate.queryForObject(QUERY_MAX_CONNECTIONS, Double.class);
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_MAX_CONNECTIONS, maxConnections != null ? maxConnections : 0);

    var activeUtilisation =
        (maxConnections == null || activeConnections == null || maxConnections == 0)
            ? 0
            : activeConnections / maxConnections;
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_UTILISATION_ACTIVE, activeUtilisation);

    var totalUtilisation =
        (maxConnections == null || totalConnections == null || maxConnections == 0)
            ? 0
            : totalConnections / maxConnections;
    metricsHandler.setCustomMetric(
        CustomMetricId.DB_CONNECTIONS_UTILISATION_TOTAL, totalUtilisation);
  }
}
